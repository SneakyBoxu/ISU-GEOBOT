 ...
 *     retrieved context, NOT generated by the Groq model (audit R5/R8).
 ...
 */
export const demoMl = {
  // DEMO_MODE never parses a real workbook: it reports a parse that changes
  // nothing, so the portal can be demonstrated without a Python service and
  // without any chance of writing to a schedule.
  async schedulePreview() {
    return {
      semester: '2026-2027-1', checksum: 'demo-checksum',
      parsed: { lecturers: 37, blocks: 425, echague: 365, santiago: 60 },
      stored: { lecturers: 37, blocks: 425 },
      diff: { added: 0, removed: 0, unchanged: 425 },
      new_lecturers: [], sample: [],
    };
  },

  async documentPreview() {
    return {
      kind: 'document', title: 'demo-document', doc_type: 'other',
      checksum: 'demo-checksum', characters: 0, chunks: 0,
      max_tokens: 0, ceiling: 220, replaces: [], preview: [],
    };
  },

  async documentApply() {
    return { applied: false, demo: true, error: 'DEMO_MODE does not write documents' };
  },

  async scheduleApply() {
    return { applied: false, demo: true, error: 'DEMO_MODE does not write schedules' };
  },
