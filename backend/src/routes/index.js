  ...
  res.json({
    ...
    demoNotice: config.demoMode
      ? 'Demonstration mode. Availability comes from a deterministic schedule '
        ...
        + 'templates over retrieved text, not generated by the Groq model. '
...
// ---------------------------------------------------------------------------
// Guard portal -- REMOVED
//
// The physical-guard presence log is gone from the running system. It fed one
// thing: a deterministic 'confirmed_off_campus' override that bypassed the
// classifier. With a synthetic cohort nobody walks past a desk, so the only
// rows it ever held were 46 created in a five-minute window while someone
// tested the portal -- test data marked 'real', which is worse than no data.
//
// resolvePresence() now always reports 'unknown', which is the neutral state
// the design already had: "no log today is NOT the same as off campus, proceed
// to the classifier" (audit F-07). Nothing downstream needed changing.
//
// The tables are left in place, unread. guard_presence_event is append-only
// with a foreign key from faculty and rows referenced by faculty_validation,
// so dropping it is a migration with real blast radius and no benefit.
// ---------------------------------------------------------------------------
...
/**
 * The consented roster a validator picks from.
 *
 * This used to be served by /guard/roster, so removing the guard portal took
 * the validation portal's faculty list with it. The dependency was accidental:
 * the panel only ever wanted names, and the guard endpoint happened to return
 * them alongside presence state it did not use. It has its own endpoint now,
 * carrying no presence field at all.
 */
api.get('/validate/roster', requireAuth, requireRole('validator', 'admin', 'researcher'),
  async (_req, res, next) => {
    try {
      const snapshot = await preloadOfflineSnapshot();
      res.json({
        roster: (snapshot.faculty ?? []).map((f) => ({
          facultyId: f.id,
          name: f.full_name,
...
api.get('/me', requireAuth, (req, res) => {
  res.json({
    ...
    can: {
      ...
      // logPresence removed with the guard portal.
