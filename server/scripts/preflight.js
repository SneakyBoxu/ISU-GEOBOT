...
if (!url) report(FAIL, 'SUPABASE_URL is not set', 'server/.env — Settings > API in your Supabase project.');
...
async function checkDatabase() {
  ...
  try {
    ...
    report(FAIL, `cannot read schema "${schema}"`,
      `${err.message}. Run db/schema.sql, functions.sql and policies.sql in the SQL editor.`);
    return;
  }
  ...
  try {
    ...
    try {
      ...
      soft(WARN, 'poi.icon column missing', 'Run db/migrations/004_poi_icon.sql (optional).');
    }

    const published = data.filter((p) => p.is_published !== false);
    if (published.length === 0) {
      report(FAIL, 'no published campus locations',
        'Run db/migrations/003_campus_locations.sql — the map will be empty without it.');
    } else {
  ...
  try {
    ...
    if (!count) {
      soft(WARN, 'the retrieval corpus is empty',
        'Run: cd ml && python ingest.py --place-cards --origin real');
    } else {
  ...
  try {
    ...
    if (unprotected.length) {
      report(FAIL, `${unprotected.length} tables without forced row-level security`,
        `${unprotected.map((r) => r.table_name).join(', ')} — re-run db/policies.sql.`);
    } else {
    ...
    soft(WARN, 'could not run rls_audit()', `${err.message}. Run db/policies.sql.`);
  }
}
  ...
  try {
    ...
    if (!h.rf_ready) {
      soft(WARN, 'no trained Random Forest',
        'Availability questions return 503 until train_rf.py runs. Correct before Phase 10.');
    }
  } catch {
    soft(WARN, `ML service unreachable at ${mlBase}`,
      'Start it with: cd ml && python app.py. The MAP works without it; chat does not.');
  }
...
function finish() {
  ...
  console.log('The campus map is ready to run live. Set DEMO_MODE=false in server/.env.');
