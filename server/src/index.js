...
import { log, ml } from './lib/clients.js';
  ...
  try {
    ...
    if (!health.rf_ready) {
      log.warn(
        ...
        'train_rf.py has run against real data. This is the correct state ' +
        'before training — a placeholder model must never be shipped (audit R6).',
      );
    }
  } catch {
    log.warn(`ML service unreachable at ${config.ml.baseUrl}. Start ml/app.py.`);
  }
