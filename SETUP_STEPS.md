...
`server/.env` already has `DEMO_MODE=true`. Nothing else is required.
...
> The `service_role` key bypasses all security. It goes in `server/.env` only —
> never in `web/.env`, never in a `VITE_` variable, never committed.

Optional, to see the real database working before real data arrives:
- [ ] Run `db/seed/001_demo_data.sql` (every row is marked synthetic)
...
2. [ ] ```bash
       ...
       cp .env.example .env
       ```
3. [ ] Put the Supabase connection string into `ml/.env` as `DATABASE_URL`.
4. [ ] ```bash
       python app.py
       ```
...
2. [ ] Convert each to plain `.txt` or `.md` in `ml/data/documents/`.
       Name them so the type is detectable: `memo-*.txt`, `calendar-*.txt`,
       `handbook-*.txt`.
3. [ ] ```bash
       cd ml && python ingest.py --path ./data/documents --origin real
       ```
...
3. [ ] Run the roles-and-locations migration if you have not already:
       ```bash
       psql "$DATABASE_URL" -f db/migrations/002_roles_and_locations.sql
       ```
...
- [ ] Restart `ml/app.py` so it picks up the new model
...
- [ ] `npm test` in `server/` passes — this is your evidence the masking
      protocol works
...
| Privacy tests | `cd server && npm test` |
| Ingest docs | `cd ml && python ingest.py --path ./data/documents --origin real` |
| Place-cards | `cd ml && python ingest.py --place-cards --origin real` |

| Problem | Cause |
|---|---|
| Banner still showing | `DEMO_MODE` is still `true` in `server/.env` |
